...
You work in `.claude/worktrees/agent-c2-builder/` on branch `feat/agent-c2-builder`. From that directory:

- **Only write files inside `packages/agents/builder/`.** Root files (`pnpm-workspace.yaml`, root `package.json`, `pnpm-lock.yaml`, `tests/contracts/`) are frozen for Phase A. If you need a new runtime dep, add it via `pnpm --filter @autobiz/agent-builder add <dep>` — pnpm updates the root lockfile in your branch only.
...
- **Merge gate** (must be green before push):
  ```bash
  pnpm --filter @autobiz/agent-builder build
  pnpm --filter @autobiz/agent-builder test:contracts
  ```
- **Commit cadence:** one commit per completed checkbox. Small commits, clear messages.
- **Push:** `git push -u origin feat/agent-c2-builder`
