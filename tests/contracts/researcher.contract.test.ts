...
describe("agent-researcher contract", () => {
  it("stub emits a thought and a terminal result via HTTP", async () => {
    const orch = await startMockOrchestrator();
    try {
      const ctx = patchOrchUrl(readFixture("contexts/researcher.json"), orch.url);
      const res = await runAgent("researcher", ctx);
      ...
      expect(orch.events.every((e) => e.agent === "researcher")).toBe(true);
