...
describe("agent-revenue-watcher contract", () => {
  it("stub emits a thought and a terminal result via HTTP", async () => {
    const orch = await startMockOrchestrator();
    try {
      const ctx = patchOrchUrl(readFixture("contexts/revenue-watcher.json"), orch.url);
      const res = await runAgent("revenue-watcher", ctx);
      ...
      expect(orch.events.every((e) => e.agent === "revenue-watcher")).toBe(true);
